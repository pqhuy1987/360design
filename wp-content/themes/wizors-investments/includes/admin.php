...
if ( !function_exists( 'wizors_investments_admin_notice' ) ) {
	//Handler of the add_action('admin_notices', 'wizors_investments_admin_notice', 2);
...
if ( !function_exists( 'wizors_investments_callback_hide_admin_notice' ) ) {
	//Handler of the add_action('wp_ajax_wizors_investments_hide_admin_notice', 'wizors_investments_callback_hide_admin_notice');
...
if ( !function_exists( 'wizors_investments_admin_footer' ) ) {
	//Handler of the add_action('admin_footer', 'wizors_investments_admin_footer');
...
if ( !function_exists( 'wizors_investments_admin_scripts' ) ) {
	//Handler of the add_action("admin_enqueue_scripts", 'wizors_investments_admin_scripts');
	function wizors_investments_admin_scripts() {

		// Add theme styles
		wp_enqueue_style(  'wizors_investments-admin',  wizors_investments_get_file_url('css/admin.css') );

		// Links to selected fonts
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen)) {
			if (wizors_investments_allow_meta_box(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
				// Load fontello icons
				...
				wp_enqueue_style(  'wizors_investments-fontello-animation', wizors_investments_get_file_url('css/fontello/css/animation.css') );
				// Load theme fonts
		...
		wp_enqueue_script( 'wizors_investments-admin', wizors_investments_get_file_url('js/_admin.js'), array('jquery'), null, true );
	}
}
	
// Add variables in the admin mode
if ( !function_exists( 'wizors_investments_admin_localize_scripts' ) ) {
	//Handler of the add_action("admin_footer", 'wizors_investments_admin_localize_scripts');
	function wizors_investments_admin_localize_scripts() {
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		wp_localize_script( 'wizors_investments-admin', 'WIZORS_INVESTMENTS_STORAGE', apply_filters( 'wizors_investments_filter_localize_script_admin', array(
			'admin_mode' => true,
