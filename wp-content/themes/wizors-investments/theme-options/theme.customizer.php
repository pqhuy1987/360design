...
if ( !function_exists( 'wizors_investments_customizer_control_js' ) ) {
	...
	function wizors_investments_customizer_control_js() {
		wp_enqueue_style( 'wizors_investments-customizer', wizors_investments_get_file_url('theme-options/theme.customizer.css') );
		wp_enqueue_script( 'wizors_investments-customizer-color-scheme-control', wizors_investments_get_file_url('theme-options/theme.customizer.color-scheme.js'), array( 'customize-controls', 'iris', 'underscore', 'wp-util' ), null, true );
		wp_localize_script( 'wizors_investments-customizer-color-scheme-control', 'wizors_investments_color_schemes', wizors_investments_storage_get('schemes') );
		wp_localize_script( 'wizors_investments-customizer-color-scheme-control', 'wizors_investments_theme_fonts', wizors_investments_storage_get('theme_fonts') );
		wp_localize_script( 'wizors_investments-customizer-color-scheme-control', 'wizors_investments_customizer_vars', array(
		...
		wp_localize_script( 'wizors_investments-customizer-color-scheme-control', 'wizors_investments_dependencies', wizors_investments_get_theme_dependencies() );
	}
}
...
if ( !function_exists( 'wizors_investments_customizer_preview_js' ) ) {
	...
	function wizors_investments_customizer_preview_js() {
		wp_enqueue_script( 'wizors_investments-customize-preview', wizors_investments_get_file_url('theme-options/theme.customizer.preview.js'), array( 'customize-preview' ), null, true );
	}
}
	...
	function wizors_investments_customizer_css_template() {
		...
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<' . esc_html($tmpl_holder) . ' type="text/html" id="tmpl-wizors_investments-color-scheme-'.esc_attr($scheme).'">'
...
if ( !function_exists('wizors_investments_init_meta_box') ) {
	add_action( 'after_setup_theme', 'wizors_investments_init_meta_box' );
	function wizors_investments_init_meta_box() {
		if ( is_admin() ) {
			add_action("admin_enqueue_scripts", 'wizors_investments_add_meta_box_scripts');
			add_action('save_post',			'wizors_investments_save_meta_box');
			add_action('add_meta_boxes',	'wizors_investments_add_meta_box');
		}
	}
}
	
// Load required styles and scripts for admin mode
if ( !function_exists( 'wizors_investments_add_meta_box_scripts' ) ) {
	add_action("admin_enqueue_scripts", 'wizors_investments_add_meta_box_scripts');
	function wizors_investments_add_meta_box_scripts() {
		// If current screen is 'Edit Page' - load fontello
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen) && wizors_investments_allow_meta_box(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
			wp_enqueue_style( 'wizors_investments-fontello',  wizors_investments_get_file_url('css/fontello/fontello-embedded.css') );
			wp_enqueue_script('jquery-ui-tabs', false, array('jquery', 'jquery-ui'), null, true);
			wp_enqueue_script( 'wizors_investments-meta-box', wizors_investments_get_file_url('theme-options/theme.meta-box.js'), array('jquery'), null, true );
			wp_localize_script( 'wizors_investments-meta-box', 'wizors_investments_dependencies', wizors_investments_get_theme_dependencies() );
		}
	}
}
...
if (!function_exists('wizors_investments_allow_meta_box')) {
	function wizors_investments_allow_meta_box($post_type) {
		return apply_filters('wizors_investments_filter_allow_meta_box', in_array($post_type, array('page', 'post')), $post_type);
	}
}

// Add meta box
if (!function_exists('wizors_investments_add_meta_box')) {
	//Handler of the add_action('add_meta_boxes', 'wizors_investments_add_meta_box');
	function wizors_investments_add_meta_box() {
		global $post_type;
		if (wizors_investments_allow_meta_box($post_type)) {
			add_meta_box(sprintf('wizors_investments_meta_box_%s', $post_type), esc_html__('Theme Options', 'wizors-investments'), 'wizors_investments_show_meta_box', $post_type, 'advanced', 'default');
		}
	}
}

// Callback function to show fields in meta box
if (!function_exists('wizors_investments_show_meta_box')) {
	function wizors_investments_show_meta_box() {
		...
		if (wizors_investments_allow_meta_box($post_type)) {
			...
			foreach ($WIZORS_INVESTMENTS_STORAGE['options'] as $k=>$v) {
				...
				$v['val'] = isset($meta[$k]) ? $meta[$k] : 'inherit';
				$tabs_content[$v['override']['section']] .= wizors_investments_show_meta_box_field($k, $v);
			}
			if (count($tabs_titles) > 0) {
				?>
				<div class="wizors_investments_meta_box">
					...
					<div id="wizors_investments_meta_box_tabs">
						<ul><?php
							$cnt = 0;
							foreach ($tabs_titles as $k=>$v) {
								$cnt++;
								?><li><a href="#wizors_investments_meta_box_<?php echo esc_attr($cnt); ?>"><?php echo esc_html($v); ?></a></li><?php
							}
						?></ul>
						<?php
							$cnt = 0;
							foreach ($tabs_content as $k=>$v) {
								$cnt++;
								?>
								<div id="wizors_investments_meta_box_<?php echo esc_attr($cnt); ?>" class="wizors_investments_meta_box_section">
...
if ( !function_exists('wizors_investments_show_meta_box_field') ) {
	function wizors_investments_show_meta_box_field($name, $field) {
		if ($field['type'] == 'hidden') return '';
		$inherit_state = wizors_investments_is_inherit($field['val']);
		$output = '<div class="wizors_investments_meta_box_item wizors_investments_meta_box_item_'.esc_attr($field['type']).' wizors_investments_meta_box_inherit_'.($inherit_state ? 'on' : 'off' ).'">'
						. '<h4 class="wizors_investments_meta_box_item_title">'
							. esc_html($field['title'])
							. '<span class="wizors_investments_meta_box_inherit_lock" id="wizors_investments_meta_box_inherit_'.esc_attr($name).'"></span>'
						. '</h4>'
						. '<div class="wizors_investments_meta_box_item_data">'
							. '<div class="wizors_investments_meta_box_item_field" data-param="'.esc_attr($name).'"'.(!empty($field['linked']) ? ' data-linked="'.esc_attr($field['linked']).'"' : '').'>';
		if ($field['type']=='checkbox') {
			$output .= '<label class="wizors_investments_meta_box_item_label">'
						. '<input type="checkbox" name="wizors_investments_meta_box_field_'.esc_attr($name).'" value="1"'.($field['val']==1 ? ' checked="checked"' : '').' />'
						. esc_html($field['title'])
					. '</label>';
		} else if ($field['type']=='switch' || $field['type']=='radio') {
			foreach ($field['options'] as $k=>$v) {
				$output .= '<label class="wizors_investments_meta_box_item_label">'
							. '<input type="radio" name="wizors_investments_meta_box_field_'.esc_attr($name).'" value="'.esc_attr($k).'"'.($field['val']==$k ? ' checked="checked"' : '').' />'
							. esc_html($v)
						. '</label>';
			}
		} else if ($field['type']=='text') {
			$output .= '<input type="text" name="wizors_investments_meta_box_field_'.esc_attr($name).'" value="'.esc_attr(wizors_investments_is_inherit($field['val']) ? '' : $field['val']).'" />';
		} else if ($field['type']=='textarea') {
			$output .= '<textarea name="wizors_investments_meta_box_field_'.esc_attr($name).'">'.esc_html(wizors_investments_is_inherit($field['val']) ? '' : $field['val']).'</textarea>';
		} else if ($field['type']=='select') {
			$output .= '<select size="1" name="wizors_investments_meta_box_field_'.esc_attr($name).'">';
			foreach ($field['options'] as $k=>$v) {
				$output .= '<option value="'.esc_attr($k).'"'.($field['val']==$k ? ' selected="selected"' : '').'>'.esc_html($v).'</option>';
			}
			$output .= '</select>';
		} else if (in_array($field['type'], array('image', 'media', 'video', 'audio'))) {
			$output .= '<input type="text" id="wizors_investments_meta_box_field_'.esc_attr($name).'" name="wizors_investments_meta_box_field_'.esc_attr($name).'" value="'.esc_attr(wizors_investments_is_inherit($field['val']) ? '' : $field['val']).'" />'
					. wizors_investments_show_custom_field('wizors_investments_meta_box_field_'.esc_attr($name).'_button', array(
														'type'				=> 'mediamanager',
														'data_type'			=> $field['type'],
														'linked_field_id'	=> 'wizors_investments_meta_box_field_'.esc_attr($name)),
														null)
					. '<div class="wizors_investments_meta_box_field_preview">'
						. (wizors_investments_is_inherit($field['val']) ? '' : ($field['val'] && $field['type']=='image' ? '<img src="' . esc_url($field['val']) . '" alt="">' : basename($field['val'])))
					. '</div>';
		}
		$output .= '<div class="wizors_investments_meta_box_inherit_cover'.(!$inherit_state ? ' wizors_investments_hidden' : '').'">'
							. '<span class="wizors_investments_meta_box_inherit_label">' . esc_html__('Inherit', 'wizors-investments') . '</span>'
							. '<input type="hidden" name="wizors_investments_meta_box_inherit_'.esc_attr($name).'" value="'.esc_attr($inherit_state ? 'inherit' : '').'" />'
						. '</div>'
					. '</div>'
					. '<div class="wizors_investments_meta_box_item_description">'
...
if (!function_exists('wizors_investments_save_meta_box')) {
	//Handler of the add_action('save_post', 'wizors_investments_save_meta_box');
	function wizors_investments_save_meta_box($post_id) {

		// verify nonce
		if ( !wp_verify_nonce( wizors_investments_get_value_gp('meta_box_post_nonce'), admin_url() ) )
			return $post_id;
		...
		foreach ($options as $k=>$v) {
			...
			if (!empty($_POST['wizors_investments_meta_box_inherit_' . $k])) continue;
			// Get option value from POST
			$meta[$k] = isset($_POST['wizors_investments_meta_box_field_' . $k])
							? $_POST['wizors_investments_meta_box_field_' . $k]
							: ($v['type']=='checkbox' ? 0 : '');
		}
