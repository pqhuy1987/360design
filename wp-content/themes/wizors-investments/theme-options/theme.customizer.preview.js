...
( function( $ ) {
	"use strict";
	var $style = $('#wizors_investments-color-scheme-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		$style = $('head').append( '<'+'style type="text/css" id="wizors_investments-color-scheme-css" />' ).find('#wizors_investments-color-scheme-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id == 'body_style') {
				...
				$('.sc_layouts_logo .logo_text').html( wizors_investments_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( wizors_investments_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( wizors_investments_prepare_macros(val) );
